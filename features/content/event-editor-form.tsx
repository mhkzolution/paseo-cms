...
export function EventEditorForm({
  ...
  const resolver = zodResolver(eventSchema) as unknown as Resolver<EventFormValues>;
  ...
  } = useForm<EventFormValues>({ resolver, defaultValues, shouldUnregister: false });
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex max-w-6xl flex-col gap-5">
      ...
      <section className={cn("grid gap-5 md:grid-cols-2", activeTab !== "taxonomy" && "hidden")}>
        ...
        <Field label="New tags" error={errors.newTags?.message}>
          ...
          <p className="text-xs font-normal text-muted">New tags are created and linked when you save the event.</p>
        </Field>
      ...
      <section className={cn("grid gap-4 md:grid-cols-2", activeTab !== "seo" && "hidden")}>
        ...
        <Field label="Canonical URL">
          <input className={inputClass} placeholder="Defaults to event URL" {...register("seo.canonicalUrl")} />
        </Field>
      ...
      <section className={cn("grid gap-5 md:grid-cols-2", activeTab !== "links" && "hidden")}>
        <CheckboxGroup label="Related events" options={events} register={register("relatedEventIds")} />
        <div className="grid gap-2">
          <div className="flex items-center justify-between">
          ...
          {alternates.fields.map((field, index) => (
            <div key={field.id} className="grid gap-2 rounded-md border border-border p-4">
              ...
              <input className={inputClass} placeholder="https://example.com/en/events/weekend-market" {...register(`alternates.${index}.url`)} />
