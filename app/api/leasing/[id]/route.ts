...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  const submission = await prisma.leasingSubmission.update({ where: { id }, data: parsed.data });
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  ...
  const { id } = await params;
  await prisma.leasingSubmission.update({ where: { id }, data: { deletedAt: new Date() } });
